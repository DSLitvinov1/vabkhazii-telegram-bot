...
def detect_issue(text):
    ...
    if any(x in t for x in ['путает буквы','зеркалит буквы','ошибки на письме']): return 'письмо и дисграфические ошибки'
    if 'заика' in t: return 'заикание'
    if any(x in t for x in ['говорит невнятно','невнятно говорит','говорит непонятно','непонятно говорит','нечеткая речь','нечёткая речь']): return 'разборчивость речи'
    if any(x in t for x in ['мало слов','несколько слов','маленький словарный запас','не говорит слова']): return 'словарный запас и развитие речи'
    if any(x in t for x in ['глотает окончания','переставляет слоги']): return 'слоговая структура речи'
...
def draft_reply(text):
    ...
    meta=extract(text)
    lead=f'Здравствуйте! Можно помочь с запросом по теме «{issue}».'
    if meta['age'] is None:
        question=' Подскажите, пожалуйста, возраст ребёнка и что сейчас больше всего беспокоит в речи?'
    else:
        question=f' Ребёнку {format_age(meta["age"])} {age_word(meta["age"])} — подскажите, пожалуйста, что сейчас больше всего беспокоит в речи и занимались ли уже со специалистом?'
    finish=' После этого смогу сориентировать по подходящему формату занятий.'
    if meta['mode']=='онлайн':
        finish=' После уточнений смогу сориентировать по занятиям онлайн.'
...
def draft_market_reply(text):
    issue=detect_issue(text)
    meta=extract(text)
    parts=[f'Здравствуйте! Могу помочь с запросом по теме «{issue}».']
    if meta['age'] is None:
